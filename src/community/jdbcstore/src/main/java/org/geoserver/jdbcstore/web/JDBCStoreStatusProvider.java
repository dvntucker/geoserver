...
public class JDBCStoreStatusProvider implements
        GeoServerHomePageContentProvider {
    
    JDBCResourceStoreProperties config;
    
    public JDBCStoreStatusProvider(JDBCResourceStoreProperties config) {
        super();
        this.config = config;
    }
    ...
    public Component getPageBodyComponent(String id) {
        ...
        if (secMgr.checkAuthenticationForAdminRole() && config.isEnabled()) { 
            return new JDBCStoreStatusPanel(id, config);
        }
